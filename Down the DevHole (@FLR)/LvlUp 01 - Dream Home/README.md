...
I also ended up writing my own version of `docker pull` to pull and inspect images layer-by-layer via the Nexus api instead of the standard registry API since I didn't know how to access the registry at the time. It might be useful some day down the line, so it's in `pull.py`.